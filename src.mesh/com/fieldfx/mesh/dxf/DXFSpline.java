...
public class DXFSpline extends DXFObject {
  ...
  public void convert( MeshBuilder d )
  {
    ...
    float lengthEstimate       = MathHelper.estimateBezierLength( pa, pb, pc, pd );
    ...
    //d.pushMatrix();
      for( int i=0; i<segments; ++i ) {
        float t0 = MathHelper.map( i,   0, segments, 0, 1 );
        float t1 = MathHelper.map( i+1, 0, segments, 0, 1 );
        
        float x0 = MathHelper.bezierPoint( pa.x, pb.x, pc.x, pd.x, t0 );
        float y0 = MathHelper.bezierPoint( pa.y, pb.y, pc.y, pd.y, t0 );
        float z0 = MathHelper.bezierPoint( pa.z, pb.z, pc.z, pd.z, t0 );
        
        float x1 = MathHelper.bezierPoint( pa.x, pb.x, pc.x, pd.x, t1 );
        float y1 = MathHelper.bezierPoint( pa.y, pb.y, pc.y, pd.y, t1 );
        float z1 = MathHelper.bezierPoint( pa.z, pb.z, pc.z, pd.z, t1 );
