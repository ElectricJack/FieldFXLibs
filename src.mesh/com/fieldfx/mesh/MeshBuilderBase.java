...
import com.fieldfx.math.MathHelper;

import processing.core.PGraphics;
import processing.core.PApplet;


public class MeshBuilderBase implements MeshBuilder {

  protected   Face                 working = null;
  protected   Mesh                 mesh;
  ...
  public void  beginPoly   ( )                            { working = new Face(); }
  public void  endPoly     ( )                            { if( working != null ) mesh.add( working ); }
  
 // ------------------------------------------------------------------------------------------------------------- //
  public void  beginObject( String name ) {
    activeObject = new MeshBuilderStore( name );
  }
 // ------------------------------------------------------------------------------------------------------------- //
  public void  endObject( ) {
    ...
    objectsByName.put( activeObject.name, activeObject );
    activeObject = null;
  }
  // ------------------------------------------------------------------------------------------------------------- //
  public void  instance( String name ) {
    MeshBuilderStore mbs = objectsByName.get( name );
    if( mbs != null )
      write( mbs );
  }
  // ------------------------------------------------------------------------------------------------------------- //
  public void  write( Convertible out ) {
    out.convert( this );
  }
  // ------------------------------------------------------------------------------------------------------------- //
  ...
  public void ellipse( float x, float y, float d1, float d2 )
  {
    ...
    float   length   = arcLengthAprox(x,y,d1,d2,0,2*MathHelper.PI);
    int     segments = (int)(length * resolution);
    
    beginPoly();
      for( int i=0; i<segments; ++i )
      {
        float a = MathHelper.map( i, 0, segments-1, 0, 2*MathHelper.PI );
        pos.set( x + MathHelper.cos(a) * d1 * 0.5f
               , y + MathHelper.sin(a) * d2 * 0.5f, 0.f );
  ...
  public void arc( float x, float y, float d1, float d2, float a1, float a2 )
  {
    ...
    beginPoly();
      for( int i=0; i<segments; ++i ) {
        float a = MathHelper.map( i, 0, segments-1, a1, a2 );
        pos.set( x + MathHelper.cos(a) * d1 * 0.5f
               , y + MathHelper.sin(a) * d2 * 0.5f, 0.f );
  ...
  private float arcLengthAprox( float x, float y, float d1, float d2, float a1, float a2 ) {
    ...
    float arcLength = 0;
    float px = MathHelper.cos(a1) * d1 * 0.5f;
    float py = MathHelper.sin(a1) * d2 * 0.5f;
    
    for( int i=1; i<integrationSteps; ++i ) {
      float angle = MathHelper.map( i, 0, 99, a1, a2 );
      float nx    = MathHelper.cos(angle) * d1 * 0.5f;
      float ny    = MathHelper.sin(angle) * d2 * 0.5f;
      float dx    = nx - px;
      float dy    = ny - py;
            px    = nx;
            py    = ny;
